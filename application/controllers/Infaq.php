...
class Infaq extends IO_Controller
{
	...
	function load_grid(){
		...
		for($i = $iDisplayStart; $i < $end; $i++) {
			...
			$act = '<a class="btn blue btn-xs '.$class_edit.'" title="UBAH DATA" onclick="editdata(\''.$data[$i]->id_infaq.'\')">
						<i class="fa fa-edit"></i>
					<a class="btn red btn-xs '.$class_delete.'" title="HAPUS DATA" onclick="deleteData(\''.$data[$i]->id_infaq.'|'.$data[$i]->tipe.'|'.$data[$i]->nominal.'|'.$data[$i]->id_donatur.'|'.$data[$i]->saldo.'|'.'\')">
