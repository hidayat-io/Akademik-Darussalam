...
class Mkurikulumsore extends CI_Model 
{
	...
	function get_bodytable_kurikulumsore(){
		...
		$data = $this->db->query ("SELECT a.id_bidang, a.nama_bidang, b.id_matpal, b.nama_matpal, b.status
									...
									WHERE b.status = 1 and a.kategori = 'SORE' OR a.kategori = 'KITAB' order by a.id_bidang, b.id_matpal asc")->result_array();
		return $data;
	}
    ...
    function query_kurikulumsore($id_thn_ajar){
        $data = array();
		$data=$this->db->query("SELECT * from trans_kurikulum where id_thn_ajar ='$id_thn_ajar' and kategori = 'SORE'")->result_array();
		return $data;
	}

	function query_kurikulumsore_byid($id_thn_ajar){
        $data = array();
		$data=$this->db->query("SELECT * from trans_kurikulum where id_thn_ajar ='$id_thn_ajar' and kategori = 'SORE'")->row_array();
		return $data;
	}
