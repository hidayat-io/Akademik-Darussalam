...
function edit_santri(no_registrasi){
	...
	var str_url = encodeURI(base_url + "nilai/get_data_nilai_by_noregistrasi/" + no_registrasi);
	$.ajax({
...
function TambahPenilaian() {
	if ($("#add_Penilaian").valid() == true) {
		...
		// 	var str_url = encodeURI(base_url + "semester/cek_data_Penilaian/" + Penilaian);
		// 	$.ajax({
		// 		type: "POST",
		// 		url: str_url,
		// 		dataType: "html",
		...
		// 			$data = $.parseJSON(data);
		// 			if ($data != null) {
		// 				bootbox.alert("Penilaian di semester lain sudah ada");
		// 				return false;
...
function svnilai() {
	if ($("#add_nilai").valid() == true) {
		$status = $('#save_button').text();
		var iform = $('#add_nilai')[0];
...
function downloadExcel(){
	...
	window.location = base_url+'nilai/exportexcel/'+param;
}
