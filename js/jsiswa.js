...
$(document).ready(function()
{
	...
	$("#lamp_bukti").on('change', function(e) {
...
function cek_jk(){ // cek saat memilih jenis kelamin di modal keluarga
	...
	if(kategori_keluarga =='AYAH'){
		$('#jenis_kelamin_keluarga').val('PRIA');
		...
		$('#jenis_kelamin_keluarga').val('WANITA');
		if ($('#kategori_santri').val() == 'AITAM_ISLAH' || $('#kategori_santri').val() == 'AITAM_JAMIAH')
			{
				$('#nama_keluarga').attr('disabled', false);
				$('#nik_keluarga').attr('disabled', false);
	...
	if($("#add_keluarga_santri").valid()==true)
	{
		...
		if(cekItem(kdkel)==true)
		{
			...
			else
			{
					...
					$.ajax({
						url: base_url+'siswa/upload_lamp_keluarga',
						type: 'post',
						enctype: 'multipart/form-data',
						contentType: false,
						processData: false,
						...
						success: function(response){
							...
							if(response!=null)
							{
										...
										content_data 	+= ' onclick="hapusItem(\''+kdkel+'\'.\''+resp.name+'\')"><i class="fa fa-fw fa-trash"></i>Hapus</button></td>';
										content_data 	+= "</tr>";
		...
		else
		{
			...
			bootbox.alert("<span class='glyphicon glyphicon-exclamation-sign'></span>&nbsp;ini "+hid_jumlah_item+" sudah ada di list.");
		}
	}
...
function TambahPenyakit(){
	if($("#add_penyakit").valid()==true)
	{
		...
		var kdpenyakit 	= nama_penyakit;
		if(cekItemPenyakit(kdpenyakit)==true)
		{
					//upload file ke temp folder
					...
					$.ajax({
						url: base_url+'siswa/upload_lamp_penyakit',
						type: 'post',
						enctype: 'multipart/form-data',
						contentType: false,
						processData: false,
						...
						success: function(response){
								...
								if(response!=null){
									...
									var content_data 	= '<tr class="tb-detail" id="row'+kdpenyakit+'">';
										...
										content_data 	+= "<td>"+linkfile+"</td>";
										content_data 	+= '<td><button type="button" class="btn btn-danger btn-xs" ';
										content_data 	+= ' onclick="hapusItemPenyakit(\''+kdpenyakit+'\'.\''+resp.name+'\')"><i class="fa fa-fw fa-trash"></i>Hapus</button></td>';
										content_data 	+= "</tr>";
	...
	if($("#add_kecakapan_khusus").valid()==true)
	{
		...
		if(cekItemkckhusus(bid_studi)==true){
			...
			var content_data 	= '<tr class="tb-detail" id="row'+kd_bidstudi+'">';
				...
				content_data 	+= '<td><button type="button" class="btn btn-danger btn-xs" ';
				content_data 	+= ' onclick="hapusItemkckhusus(\''+bid_studi+'\')"><i class="fa fa-fw fa-trash"></i>Hapus</button></td>';
				content_data 	+= "</tr>";
			...
			bootbox.alert("<span class='glyphicon glyphicon-exclamation-sign'></span>&nbsp;ini "+kd_bidstudi+" sudah ada di list.");
		}
	}
}
...
function edit(no_registrasi){
	...
	$.ajax({
		...
		type:"POST",
		url:base_url+"siswa/get_data_santri/"+no_registrasi,
		dataType:"html",
		success:function(data){
	...
	$.ajax({
		...
		type:"POST",
		url:base_url+"siswa/get_data_keluarga/"+no_registrasi,
		dataType:"html",
		success:function(data){
	...
	$.ajax({
		...
		type:"POST",
		url:base_url+"siswa/get_data_penyakit/"+no_registrasi,
		dataType:"html",
		success:function(data){
	...
	$.ajax({
		...
		type:"POST",
		url:base_url+"siswa/get_data_kecakapankhusus/"+no_registrasi,
		dataType:"html",
		success:function(data){
